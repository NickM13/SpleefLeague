 ...
 * @since 4/24/2020
 */
public class BattleVersus extends Battle {
    
    public BattleVersus(CorePlugin<?> plugin, List<CorePlayer> players, Arena arena, Class<? extends BattlePlayer> battlePlayerClass) {
    ...
    protected void endRound(BattlePlayer winner) {
        chatGroup.sendMessage(winner.getCorePlayer().getDisplayName() + " won the round");
    }
    ...
    protected void endBattle(BattlePlayer winner) {
        Core.getInstance().sendMessage(arena.getMode().getChatTag());
    }
