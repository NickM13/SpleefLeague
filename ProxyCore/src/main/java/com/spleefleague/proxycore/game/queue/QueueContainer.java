...
public class QueueContainer {
    ...
    private int minTeamCount = 1;

    public QueueContainer(String mode) {
        this.mode = mode;
        queuedPlayers = new ArrayList<>();
        teamSizes = new HashSet<>();
    }

    public void setMinTeamCount(int count) {
        minTeamCount = count;
    }
     ...
     * @return
     */
    public boolean join(ProxyCorePlayer pcp, String query) {
        boolean replaced = leave(pcp);
        queuedPlayers.add(new QueuePlayer(pcp, query));
     ...
     * @return
     */
    public boolean leave(ProxyCorePlayer pcp) {
        return queuedPlayers.removeIf(queuePlayer -> queuePlayer.pcp.equals(pcp));
    }
    ...
    public List<QueuePlayer> getMatchedPlayers(int count, int teamSize) {
        ...
        ListIterator<QueuePlayer> pit = queuePlayers.listIterator();
    ...
    public void checkQueue() {
        for (int size : teamSizes) {
            if (getQueueSize() >= minTeamCount) {
                List<QueuePlayer> players = gatherPlayers(minTeamCount, size);
                if (players != null) {
    ...
    public void startMatch(List<QueuePlayer> players, String query) {
        SortedSet<ServerInfo> minigameServers = new TreeSet<>(Comparator.comparingInt(l -> l.getPlayers().size()));
        for (Map.Entry<String, ServerInfo> entry : ProxyCore.getInstance().getProxy().getServers().entrySet()) {
            if (entry.getKey().startsWith("minigame")) {
                minigameServers.add(entry.getValue());
            }
        }
        if (minigameServers.isEmpty()) {
            ProxyCore.getInstance().getLogger().warning("There are no minigame servers available right now!");
            return;
        }
        ServerInfo server = Iterables.getFirst(minigameServers, null);

        ByteArrayDataOutput output = ByteStreams.newDataOutput();
        output.writeUTF(mode);
        output.writeUTF(query);
        for (QueuePlayer qp : players) {
            if (!qp.pcp.canJoinBattle()) {
                return;
            }
        ...
        for (QueuePlayer qp : players) {
            ...
            qp.pcp.getPlayer().connect(server);
        }
        server.sendData("battle:start", output.toByteArray());
    }
